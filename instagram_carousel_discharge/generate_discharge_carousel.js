...
async function generateInstagramCarousel() {
  try {
    console.log("[System] Starting compilation for the 7-slide Instagram Carousel (1080x1350px)...");
    ...
    const filePath = path.join(__dirname, 'instagram_carousel_discharge_blueprint.html');
    ...
    // Loop through each of the 7 slides and capture screenshots of individual slide containers
    for (let i = 1; i <= 7; i++) {
    ...
    const localPdfPath = path.join(__dirname, 'instagram_carousel_discharge.pdf');
    const artifactPdfPath = path.join(artifactDir, 'instagram_carousel_discharge.pdf');
    ...
    await browser.close();
    console.log("[OK] Instagram 7-slide Carousel successfully compiled!");
  } catch (error) {
